...
import com.plugi.plugi.feature.search.filter.adapter.SizeTypeItemAdapter;
import com.plugi.plugi.feature.search.filter.interfaces.OnSizeClickListener;
import com.plugi.plugi.feature.search.filter.interfaces.OnSizeTypeClickListener;
...
public class SizeTypeFragment extends BaseFragment implements OnSizeTypeClickListener {

    private SizeTypeItemAdapter sizeTypeItemAdapter;
    private RecyclerView rec_FilterItem;
    private List<FilterIDs.SizeTypeList> sizeTypeList;
    private ShimmerFrameLayout shimmerFrameLayout;

    private TextView  ivClose , ivApply , filterName;
    FilterIDs.SizeTypeList selectSizeType;
    FilterIDs.SizeTypeList prepSizeType;
    @Nullable
    @Override
    public View onCreateView(@NotNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {

        View view = inflater.inflate(R.layout.fragment_filter_item, container, false);
        selectSizeType =(FilterIDs.SizeTypeList) getArguments().getSerializable(Constants.BUNDLE_SIZETYPE_ID);
        ...
        ivApply.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(prepSizeType.getID() !=0) {
                    ((MainActivity) getActivity()).setSelectedSizeType(prepSizeType);
                    Fragment fragment = null;
                    fragment = new FilterFragment();
    ...
    private void AssignSortList() {
        ...
        sizeTypeList.addAll(((MainActivity) getActivity()).getSizeTypeList());
        ...
        int selectedIndex = -1;
        if(selectSizeType != null) {
            //Log.d("FILTER", "AssignSortList: " + selectBrand.getName());
            selectedIndex = sizeTypeList.indexOf(selectSizeType);
        }

        sizeTypeItemAdapter = new SizeTypeItemAdapter( getActivity() ,sizeTypeList , selectedIndex , this);
        rec_FilterItem.setAdapter(sizeTypeItemAdapter);
    }
    public void loadData()
    {

        sizeTypeItemAdapter.notifyDataSetChanged();
    ...
    @Override
    public void onSizeTypeClicked(FilterIDs.SizeTypeList contact, int position) {
        if(selectSizeType != null) {
            if (!contact.get$id().equals(selectSizeType.get$id())) {
